...
class OperationButtonDelete:
    ...
    @staticmethod
    def delete_note_button(current_labels):
        answer = messagebox.askokcancel("Delete Note", "Are you sure you want to delete the note?")
        if answer:
            NotesDatabaseAction.delete_note("notes_info", note_id=current_labels[0].cget("text"))

    def open_new_window(self, current_labels):
        new_window = Toplevel(self.root)
        new_window.title(f"{current_labels[4].cget('text')}")
