...
class OperationButtonAdd:
    ...
    def add_note_button(self, main_win):
        ...
        header_label = Label(main_win, text="ADD A NEW NOTE DATA", bg=QuickNotesApp.PALETTE["main"]["3color"],
        ...
        save_button = Button(main_win, text="Save", bg=QuickNotesApp.PALETTE["secondary"]["1color"],
                             fg=QuickNotesApp.PALETTE["text"]["1color"], activebackground=QuickNotesApp.PALETTE["secondary"]["1color"],
                             border=3, relief="flat", activeforeground=QuickNotesApp.PALETTE["text"]["2color"], command=saving_comm)
        save_button.place(x=600, y=450)

        category_name_label = Label(main_win, text="CATEGORY(*):", bg=QuickNotesApp.PALETTE["main"]["3color"] ,
                                    fg=QuickNotesApp.PALETTE["text"]["1color"],
                                    font=("Arial", 13, "bold"))
        category_name_label.place(x=500, y=130)

        # Creating a frame for selecting categories
        select_category_frame = Frame(main_win)
        select_category_frame.place(x=500, y=179)
